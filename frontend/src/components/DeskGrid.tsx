...
export function DeskGrid({ data }: { data: DeskProps[] }) {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 text-left" role="grid" aria-label="Desk analysis grid">
      {data.map((desk) => (
        <DeskCard key={desk.ticker} desk={desk} role="gridcell" />
      ))}
    </div>
  )
}
