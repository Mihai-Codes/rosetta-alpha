...
with open("tailwind.config.js", "r") as f:
    tw = f.read()
if "'glow-red': '0 0 24px" not in tw:
...
layout = re.sub(
    r'<div className="w-full max-w-\[1440px\] mx-auto px-4 sm:px-8 lg:px-12 mt-12 pt-8 border-t border-white/\[0\.02\] flex items-center justify-between flex-wrap gap-4 text-\[10px\] text-text-tertiary uppercase tracking-\[0\.25em\]">\s*<span>© 2026 Rosetta Alpha</span>\s*<span className="italic text-text-secondary">.*?</span>\s*</div>',
...
hero = hero.replace("hover:border-brand-red/40 text-brand-red", "hover:border-brand-red/60 text-brand-red")

hero = hero.replace("border-white/[0.05] mb-10", "border-brand-red/40 shadow-[0_0_15px_rgba(216,43,43,0.15)] mb-10")
hero = hero.replace('<div className="absolute inset-0 hero-grid-bg" aria-hidden />', '')

with open("src/components/HeroSection.tsx", "w") as f:
    f.write(hero)

# 5. Update AllWeatherChart.tsx
...
thesis = thesis.replace('text-justify hyphens-auto', 'text-left pr-4')
thesis = thesis.replace('pl-4 border-l border-border', 'pl-4 border-l-2 border-border/50')

thesis = re.sub(
    r'<p className="text-sm text-text-primary font-light leading-relaxed mb-2 text-left pr-4[^>]*>\s*\{block\.analysis\}\s*</p>',
...
thesis = re.sub(
    r'<p className="text-sm text-text-secondary font-light leading-relaxed mb-2 pl-4 border-l-2 border-border/50 text-left pr-4[^>]*>\s*\{block\.analysis_en\}\s*</p>',
